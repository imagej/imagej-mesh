...
public class MeshConnectedComponents
{
			...
			for ( int vid = nextClearBit; vid < nVertices; vid++ )
			{
				...
				while ( !queue.isEmpty() )
				{
					...
					for ( int i = 0; i < nts; i++ )
					{
						...
						visit( ( int ) triangles.vertex0( t ), visited, queue );
						visit( ( int ) triangles.vertex1( t ), visited, queue );
						visit( ( int ) triangles.vertex2( t ), visited, queue );
					}
				}
	...
	private static final TIntObjectHashMap< TIntArrayList > triangleMap( final Mesh mesh )
	{
		...
		{
			addToMap( map, ( int ) triangles.vertex0( tid ), tid );
			addToMap( map, ( int ) triangles.vertex1( tid ), tid );
			addToMap( map, ( int ) triangles.vertex2( tid ), tid );
		}
		return map;
	}
	...
	private static final class MyCCIterator implements Iterator< BufferMesh >
	{
				...
				for ( int vid = currentStartVertex; vid < nVertices; vid++ )
				{
					...
					while ( !queue.isEmpty() )
					{
						...
						for ( int i = 0; i < nts; i++ )
						{
							final int t = ts.get( i );
							boolean add = false;
							add = visit( ( int ) triangles.vertex0( t ), visited, queue ) || add;
							add = visit( ( int ) triangles.vertex1( t ), visited, queue ) || add;
							add = visit( ( int ) triangles.vertex2( t ), visited, queue ) || add;
		...
		private BufferMesh makeCC( final TIntHashSet cc )
		{
			...
			final net.imagej.mesh.nio.BufferMesh.Vertices outVertices = out.vertices();
			final net.imagej.mesh.nio.BufferMesh.Triangles outTriangles = out.triangles();
