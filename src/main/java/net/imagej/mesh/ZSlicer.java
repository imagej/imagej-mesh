...
public class ZSlicer
{
	...
	private static final class Segment implements Comparable< Segment >
	{
		...
		private Segment( final double xa, final double ya, final long ea, final long eb, final double nx, final double ny )
		{
			this.xa = xa;
			this.ya = ya;
			this.ea = ea;
	...
	public static final class Contour
	{
		...
		/**
		 * Grows this contour by finding one segment from the specified list
		 * that connect to its head. Sets {@link #isClosed()} to
		 * <code>true</code> when the segment found connects to its tail.
		 * 
		 * @param segments
		 *            the list of segments to grow from. At most one is removed
		 *            from this list.
		...
		/**
		 * Determine whether this contour surrounds the interior of the mesh or
		 * the exterior. Sets the {@link #isInterior} field.
		 */
	...
	private static final double minZ( final Vertices vertices, final long v0, final long v1, final long v2, final double eps )
	{
	...
	private static final double maxZ( final Vertices vertices, final long v0, final long v1, final long v2, final double eps )
	{
		final double z0 = mround( vertices.z( v0 ), eps, 2, 0 );
		final double z1 = mround( vertices.z( v1 ), eps, 2, 0 );
		final double z2 = mround( vertices.z( v2 ), eps, 2, 0 );
