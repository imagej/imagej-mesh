 ...
 */
public class Vertex3 extends
	PoolObject<Vertex3, Vertex3Pool, BufferMappedElement> implements
	RealLocalizable
{
	...
	public Vertex3 init(final float x, final float y, final float z) {
		return init(x, y, z, 0, 0, 0, 0, 0, 0);
	}

	public Vertex3 init(final float x, final float y, final float z,
		final float nx, final float ny, final float nz, final float u,
		final float v, final float w)
	{
		// like this:
	...
	public String toString() {
		return String.format(
			"v(%.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f, %.2f)", getX(), getY(),
			getZ(), getNX(), getNY(), getNZ(), getU(), getV(), getW());
	}
	...
	public void setFloatPosition(final float val, final int d) {
		position.set(d, val);
	}

	public void setDoublePosition(final double val, final int d) {
